...
class EloquentRepository extends BaseRepository
{
    ...
    public function paginate($perPage = null, $attributes = ['*'], $pageName = 'page', $page = null)
    {
        ...
        return $this->executeCallback(static::class, __FUNCTION__, array_merge(func_get_args(), compact('page')), function () use ($perPage, $attributes, $pageName, $page) {
            $prepareQuery = $this->prepareQuery($this->createModel());
            if (in_array($perPage, config('hub.no_limit', [-1, "all"]))) {
                $perPage = $prepareQuery->count();
            }

            if (!is_numeric($perPage) || $perPage < -1) {
                $perPage = config('hub.per_page', 10);
            }

            if ($perPage > config('hub.max_request', 1000)) {
                $perPage = config('hub.max_request', 1000);
            }
            return $prepareQuery->paginate($perPage, $attributes, $pageName, $page);
    ...
    public function simplePaginate($perPage = null, $attributes = ['*'], $pageName = 'page', $page = null)
    {
        ...
        return $this->executeCallback(static::class, __FUNCTION__, array_merge(func_get_args(), compact('page')), function () use ($perPage, $attributes, $pageName, $page) {
            ...
            return $prepareQuery->simplePaginate($perPage, $attributes, $pageName, $page);
        ...
        return $this->executeCallback(static::class, __FUNCTION__, func_get_args(), function () use ($where, $attributes) {
            if (count($where) > 1) {
                foreach ($where as $key => $value) {
                    [$attribute, $operator, $value, $boolean] = array_pad($value, 4, null);
    ...
    {
        return Schema::getColumnListing($this->createModel()->getTable());
    }
}
